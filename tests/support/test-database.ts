...
export async function createTestDatabase(): Promise<TestDatabase> {
  ...
  return {
    ...
    async truncate() {
      await client.exec(
        `truncate table "item_tags", "item_recurrence", "item_suggestions", "items", "note_tags", "notes", "tags", "projects", "kitchen_inventory", "approvals", "usage_entries", "runs", "jobs", "ai_execution_profiles", "runtime_credentials", "runtime_capabilities", "runtime_role_grants", "capacity_updates", "capacity_pools", "runtimes" cascade;`,
      );
    },
    async close() {
      await client.close();
