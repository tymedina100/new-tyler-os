...
struct LibraryView: View {
    ...
    var body: some View {
        List {
            if store.knowledgeUnavailable { Text("Personal knowledge could not be refreshed. Notes and tasks remain available.").font(.caption).accessibilityIdentifier("knowledgeUnavailable") }
            if let knowledge = store.knowledge { Section {
                Label("Notion · \(knowledge.mode.capitalized)", systemImage: "link")
                Text(knowledge.health?.message ?? "Saved snapshot. Check Notion for current information.").font(.caption).foregroundStyle(.secondary).accessibilityIdentifier("knowledgeHealth")
                if let date = knowledge.asOf { Text("Latest import batch: \(date)").font(.caption).foregroundStyle(.secondary) }
            } }
            if !entries.isEmpty { Section("Personal knowledge") { ForEach(entries) { entry in NavigationLink { ScrollView { VStack(alignment: .leading, spacing: 18) { Text(entry.title).font(.title.bold()); Text(entry.sourceHealth?.reviewMessage ?? "Review timing unverified. Check the source in Notion.").font(.caption).foregroundStyle(.orange).accessibilityIdentifier("knowledgeReviewStatus"); if let imported = entry.sourceHealth?.importMessage { Text(imported).font(.caption).foregroundStyle(.secondary) }; NoteBody(text: entry.body); if let edited = entry.sourceEditedAt { Text("Source edited: \(edited)").font(.caption).foregroundStyle(.secondary) }; if let reviewed = entry.lastReviewed { Text("Last reviewed: \(reviewed)").font(.caption).foregroundStyle(.secondary) }; if let source = entry.sourceUrl, let url = URL(string: source), url.scheme == "https" { Link("Open source in Notion", destination: url) } }.padding(24) }.navigationTitle("Knowledge").navigationBarTitleDisplayMode(.inline) } label: { VStack(alignment: .leading, spacing: 5) { Text(entry.title); Text(entry.sourceHealth?.reviewMessage ?? "Review timing unverified").font(.caption).foregroundStyle(.secondary) } } } } }
